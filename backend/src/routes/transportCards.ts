...
import { transportCards, expenses, transportCardHistory, drivers, users } from "../db/schema.js";
...
router.put("/:id", authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const currentCard = db.select().from(transportCards).where(eq(transportCards.id, cardId)).get();
    ...
    const updatedCard = db.select().from(transportCards).where(eq(transportCards.id, cardId)).get();
