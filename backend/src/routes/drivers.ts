...
router.get("/:id", authenticate, (req: AuthRequest, res) => {
  try {
    ...
    const transportCard = db
      .select()
      .from(transportCards)
      .where(eq(transportCards.driverId, driver.id))
      .get();

    // Если карта есть, получаем расходы
    if (transportCard) {
      const cardExpenses = db
        .select()
        .from(expenses)
        .where(eq(expenses.transportCardId, transportCard.id))
        .all();
      const totalExpenses = cardExpenses.reduce((sum, e) => sum + (e.amount || 0), 0);
...
router.post("/", authenticate, (req: AuthRequest, res) => {
  try {
    ...
    const result = db.insert(drivers).values(data).run();
...
router.put("/:id", authenticate, (req: AuthRequest, res) => {
  try {
    ...
    db.update(drivers).set(updateData).where(eq(drivers.id, driverId)).run();

    const updatedDriver = db.select().from(drivers).where(eq(drivers.id, driverId)).get();
