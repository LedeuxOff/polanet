...
for (const role of defaultRoles) {
  const existing = db.select().from(roles).where((r) => r.code === role.code).get()
  if (!existing) {
...
const adminRole = db.select().from(roles).where((r) => r.code === 'ADMIN').get()

if (adminRole) {
  const existingAdmin = db.select().from(users).where((u) => u.email === 'admin@polanet.local').get()
