...
try {
  // Get all cars permissions
  const carsPermissions: Array<{ id: number; code: string }> = db
    .prepare("SELECT id, code FROM permissions WHERE module = 'cars'")
    .all() as Array<{ id: number; code: string }>;

  if (carsPermissions.length === 0) {
    console.log("No cars permissions found in database!");
    process.exit(1);
  }

  console.log("Found cars permissions:");
  carsPermissions.forEach((p) => console.log(`  - ${p.code} (ID: ${p.id})`));
  ...
  let addedCount = 0;
  for (const permission of carsPermissions) {
  ...
  // Verify
  console.log("\nFinal role permissions for cars module:");
  const finalCheck = db
    .prepare(
      "SELECT r.name, r.code, p.code as permission_code FROM role_permissions rp JOIN roles r ON rp.role_id = r.id JOIN permissions p ON rp.permission_id = p.id WHERE p.module = 'cars' ORDER BY r.code, p.code",
    )
