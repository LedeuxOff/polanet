...
try {
  // Get all drivers permissions
  const driversPermissions: Array<{ id: number; code: string }> = db
    .prepare("SELECT id, code FROM permissions WHERE module = 'drivers'")
    .all() as Array<{ id: number; code: string }>;

  if (driversPermissions.length === 0) {
    console.log("No drivers permissions found in database!");
    process.exit(1);
  }

  console.log("Found drivers permissions:");
  driversPermissions.forEach((p) => console.log(`  - ${p.code} (ID: ${p.id})`));
  ...
  let addedCount = 0;
  for (const permission of driversPermissions) {
  ...
  // Verify
  console.log("\nFinal role permissions for drivers module:");
  const finalCheck = db
    .prepare(
      "SELECT r.name, r.code, p.code as permission_code FROM role_permissions rp JOIN roles r ON rp.role_id = r.id JOIN permissions p ON rp.permission_id = p.id WHERE p.module = 'drivers' ORDER BY r.code, p.code",
    )
