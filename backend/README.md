...
Проект включает встроенную систему резервного копирования базы данных.

### Создание бэкапа

```bash
# Создать бэкап с автоматическим именем (с таймстемпом)
npm run db:backup

# Создать бэкап с указанным путём
npx tsx src/db/backup.ts create /tmp/my-backup.db
```

Бэкапы сохраняются в `data/backups/` с форматом имени: `polanet-backup-YYYY-MM-DDTHH-MM-SS.db`

### Восстановление из бэкапа

```bash
# Восстановить из бэкапа (автоматически создаёт бэкап текущего состояния)
npx tsx src/db/backup.ts restore data/backups/polanet-backup-2024-01-01.db
```

**Важно**: перед восстановлением автоматически создаётся бэкап текущего состояния базы данных.
...
npx tsx src/db/backup.ts delete data/backups/polanet-backup-2024-01-01.db
```

### API для использования в коде

```typescript
import { createBackup, restoreFromBackup, listBackups, deleteBackup } from "./db/backup";

// Создать бэкап
const path = createBackup(); // или createBackup("/custom/path.db")

// Восстановить из бэкапа
restoreFromBackup("data/backups/polanet-backup-2024-01-01.db");

// Получить список всех бэкапов
const backups = listBackups();
// [{ filename, path, size, createdAt }]

// Удалить бэкап
deleteBackup("data/backups/polanet-backup-2024-01-01.db");
```

### Автоматический бэкап (рекомендация)

Для регулярного автоматического бэкапа рекомендуется настроить cron-задачу:

**Linux/macOS** (crontab):

```bash
# Ежедневный бэкап в 3:00 ночи
0 3 * * * cd /path/to/backend && npx tsx src/db/backup.ts create
```

**Windows** (Task Scheduler):

```powershell
# Создать задачу с триггером каждый день в 3:00
cd C:\path\to\backend && npx tsx src/db/backup.ts create
```

### Автоматическое резервное копирование

Проект поддерживает встроенную систему автоматического резервного копирования. Бэкапы создаются автоматически при запуске сервера и далее по заданному расписанию.

#### Включение автоматического бэкапа

Установите переменную окружения `AUTO_BACKUP=true`:
...
| ----------------------- | ------------ | ------------------------------------------------- |
| `AUTO_BACKUP`           | `false`      | Включить автоматическое резервное копирование     |
| `AUTO_BACKUP_INTERVAL`  | `3600000`    | Интервал между бэкапами в мс (по умолчанию 1 час) |
| `AUTO_BACKUP_MAX_COUNT` | `24`         | Максимальное количество бэкапов для хранения      |

#### Примеры использования

```bash
# Бэкап каждый час (по умолчанию)
AUTO_BACKUP=true npm run dev

# Бэкап каждые 30 минут
AUTO_BACKUP=true AUTO_BACKUP_INTERVAL=1800000 npm run dev

# Бэкап каждый час, хранить последние 48 бэкапов
AUTO_BACKUP=true AUTO_BACKUP_MAX_COUNT=48 npm run dev

# Бэкап каждые 6 часов
AUTO_BACKUP=true AUTO_BACKUP_INTERVAL=21600000 npm run dev
```

#### Как это работает

1. При запуске сервера автоматически создаётся первый бэкап
2. Далее бэкапы создаются через заданный интервал
3. Автоматически удаляются старые бэкапы, если их количество превышает `AUTO_BACKUP_MAX_COUNT`
4. При остановке сервера (SIGINT/SIGTERM) система корректно завершает работу

> **Примечание**: Бэкапы сохраняются в `data/backups/` с форматом имени: `polanet-backup-YYYY-MM-DDTHH-MM-SS.db`
...
| `npm run db:backup`        | Создать резервную копию БД             |
| `npm run db:backup:list`   | Список всех бэкапов                    |
| `npm run db:restore`       | Восстановить из бэкапа                 |
| `npm run db:backup:delete` | Удалить бэкап                          |
